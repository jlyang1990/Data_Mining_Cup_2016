...
y_pred_prob = tapply(y_pred_prob_feat[(n_train+1):n_tot], y_index_test, sum)
y_pred = round(y_pred_prob)

cat(paste(mean(scores), sd(scores), '\n'))

save(y, y_index, scores, y_pred, y_pred_prob, y_pred_prob_feat, ind_drop, file = "DMC16-expand-test-drop-gift.RData")