...
# drop feats
df_all = subset(df_all, select=-c(price_per_cust_date,
                                  price_per_cust_3date, 
                                  price_per_cust_5date, 
                                  price_per_cust_7date, 
                                  price_per_cust_11date, 
                                  price_per_cust_15date))
...
for (i in 1:n_fold){
  ...
  bst = xgb.train(param, dtrain, nthread=36, nrounds=1e4, watchlist, early.stop.round=100, maximize=FALSE)
...
save(y, y_index, scores, y_pred, y_pred_prob, y_pred_prob_feat, ind_drop, file = "DMC16-expand-test-drop-feat.RData")